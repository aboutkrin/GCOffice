...
interface ProductSummaryToolbarProps {
  previewRef: React.RefObject<HTMLDivElement | null>;
  filename: string;
}

export function ProductSummaryToolbar({
  previewRef,
  filename,
}: ProductSummaryToolbarProps) {
  ...
  const preloadImages = useCallback(
    async (element: HTMLElement): Promise<void> => {
      ...
      const promises = Array.from(images).map(async (img) => {
        ...
        try {
          let response = await fetch(img.src, { mode: "cors" }).catch(
            () => null
          );
